# Deploy arcraiderscheats.org

Step-by-step guide to deploy the Arc Raiders Cheats static site to **arcraiderscheats.org** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **arcraiderscheats.org** DNS
...
3. Configure build settings:
   - **Project name:** `arc-raidersscheats` (existing) or create a new project
...
This runs `wrangler pages deploy dist --project-name=bestarc-raiderscheats` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **arcraiderscheats.org** as the primary custom domain on the Pages project.

### Apex (arcraiderscheats.org)
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.arcraiderscheats.org/*`
   - **Target:** `https://arcraiderscheats.org/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`arcraiderscheats.org`, `.net`, `.com`), and legacy path redirects.
...
- `https://arcraiderscheats.org/es/`
- `https://arcraiderscheats.org/arc-raiders-cheats/`
- `https://arcraiderscheats.org/arc-raiders-aimbot/`
- `https://arcraiderscheats.org/sitemap.xml`
- `https://arcraiderscheats.org/robots.txt`

Verify redirects:

- `http://arcraiderscheats.org` → `https://arcraiderscheats.org` (301)
- `https://www.arcraiderscheats.org` → `https://arcraiderscheats.org` (301)
- Legacy domains (e.g. `arcraiderscheats.org`) → `https://arcraiderscheats.org` (301)
- `/sitemap-index.xml` → `/sitemap.xml` (301)
- Legacy paths (e.g. `/fortnite-hacks/`) → Arc Raiders equivalents (301)

## 5. Google Search Console

1. Go to [Google Search Console](https://search.google.com/search-console).
2. **Add property** → choose **Domain** → enter `arcraiderscheats.org`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://arcraiderscheats.org/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `arcraiderscheats.org` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/arc-raiders-cheats/`)
   - Key landing pages (`/arc-raiders-aimbot/`, `/arc-raiders-esp/`, `/arc-raiders-cheats-2026/`, etc.)
...
- [ ] Custom domain `arcraiderscheats.org` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `arcraiderscheats.org`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://arcraiderscheats.org`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/arc-raiders-cheats/` requested for indexing
